    ...
    for (size_t i = 0; i < TestClass.num_params(); i++) {
      ...
      for (size_t n = 0; n < parameters.size(); n++) {
        ...
        var log_prob_at_lower_bound = TestClass.template log_prob
          <T0,T1,T2,T3,T4,T5,T6,T7,T8,T9>
          (p0,p1,p2,p3,p4,p5,p6,p7,p8,p9);
        EXPECT_NEAR(power * TestClass.lower_bound_vals()[i][n], log_prob_at_lower_bound.val(),1.0e-8)
    ...
    for (size_t i = 0; i < TestClass.num_params(); i++) {
      ...
      for (size_t n = 0; n < parameters.size(); n++) {
        ...
        var log_prob_at_upper_bound = TestClass.template log_prob
          <T0,T1,T2,T3,T4,T5,T6,T7,T8,T9>
          (p0,p1,p2,p3,p4,p5,p6,p7,p8,p9);
        EXPECT_NEAR(power * TestClass.upper_bound_vals()[i][n], log_prob_at_upper_bound.val(), 1.0e-8)
