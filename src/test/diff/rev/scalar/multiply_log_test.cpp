...
TEST(DiffRevScalar,multiplyLogChainVV) {
  AVAR a = 19.7;
  AVAR b = 1299.1;
...
TEST(DiffRevScalar,multiplyLogChainDV) {
  double a = 19.7;
  AVAR b = 1299.1;
  AVAR f = 2.0 * multiply_log(a,b);
...
TEST(DiffRevScalar,multiplyLogChainVD) {
  AVAR a = 19.7;
  double b = 1299.1;
  AVAR f = 2.0 * multiply_log(a,b);
...
TEST(DiffRevScalar,multiply_log_var_var) {
  AVAR a = 2.2;
  AVAR b = 3.3;
...
TEST(DiffRevScalar,multiply_log_var_double){
  AVAR a = 2.2;
  double b = 3.3;
  AVAR f = multiply_log(a,b);
  EXPECT_FLOAT_EQ(2.2*std::log(3.3),f.val()) << "Reasonable values";
...
TEST(DiffRevScalar,multiply_log_double_var){
  double a = 2.2;
  AVAR b = 3.3;
  AVAR f = multiply_log(a,b);
  EXPECT_FLOAT_EQ(2.2*std::log(3.3),f.val()) << "Reasonable values";
