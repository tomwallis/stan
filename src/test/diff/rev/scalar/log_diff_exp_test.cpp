...
TEST(DiffRevScalar,log_diff_exp_vv) {
  AVAR a = 5.0;
  AVAR b = 2.0;
...
TEST(DiffRevScalar,log_diff_exp_vd) {
  AVAR a = 5.0;
  double b = 2.0;
  AVAR f = log_diff_exp(a, b);
  EXPECT_FLOAT_EQ (std::log(std::exp(5) - std::exp(2)), f.val());
...
TEST(DiffRevScalar,log_diff_exp_dv) {
  double a = 5.0;
  AVAR b = 2.0;
  AVAR f = log_diff_exp(a, b);
  EXPECT_FLOAT_EQ (std::log(std::exp(5) - std::exp(2)), f.val());
...
TEST(DiffRevScalar,log_diff_exp_2) {
...
TEST(DiffRevScalar,log_diff_exp_exception) {
